...
#n->세로, m->가로
from copy import deepcopy #INTRODUCING - 'copy' MOUDLE!
def flip(n,m,board): 
    res=deepcopy(board) #Helps copying n-dimensional lists 
    for i in range(n):
        for j in range(m):
            if res[i][j]=='B':
                res[i][j]='W'
...
def countAll(inpuT,comp,n,m):
    ...
    min_count=32767
    #n-8, m-8 기준으로 함
    for k in range(0,n-7): #세로
        count=0
        for l in range(0,m-7): #가로
            count=0
...
res=board1_count if board1_count<board2_count else board2_count
print(res)